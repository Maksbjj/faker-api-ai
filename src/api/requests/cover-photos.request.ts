...
export class CoverPhotosRequest extends BaseRequest {
  constructor(request: APIRequestContext) {
    super(request, apiUrls.coverPhotosUrl);
  }
  ...
  async post(data: CoverPhotoPayload): Promise<APIResponse> {
    return this.request.post(this.url, { data });
  }

  async put(data: CoverPhotoPayload, id: number): Promise<APIResponse> {
  ...
  async getByBook(idBook: number): Promise<APIResponse> {
    return this.request.get(`${this.url}/books/covers/${idBook}`);
  }
}
